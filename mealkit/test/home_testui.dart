...
void testCartButton(WidgetTester tester) async
{
    await tester.pumpWidget(MaterialApp(
      home: Scaffold(
        body: Navigationbar(total: 0.0), // Render Navigationbar on the screen for testing
      ),
      routes: {
        '/cart': (context) => CheckoutPage(), // replace cartpage with the class that defines that page
      },
    ));
    ...
    expect(find.text('Basket'), findsOneWidget); //replace cart with unique word on the page
}
...
void testLoginButton(WidgetTester tester) async
{
    ...
    expect(find.byType(Login), findsOneWidget); //replace login with unique word on the page
}

void testAboutUsButton(WidgetTester tester) async
{
    await tester.pumpWidget(MaterialApp(
      home: Scaffold(
        body: Navigationbar(total: 0.0), // Render Navigationbar on the screen for testing
      ...
      routes: {
        '/about' : (context) =>  AboutUsPage(), // replace LoginPage with the class that defines the page
      },
    ));

    // Find the login button
    final Finder AboutButtonFinder = find.text('About Us');
    ...
    // Verify if navigation to login page occurred
    expect(find.byType(AboutUsPage), findsOneWidget); //replace login with unique word on the page
}

void testMenuButton(WidgetTester tester) async
{
    await tester.pumpWidget(MaterialApp(
      home: Scaffold(
        body: Navigationbar(total: 0.0), // Render Navigationbar on the screen for testing
      ...
      routes: {
        '/menu': (context) => MenuPage(), // replace LoginPage with the class that defines the page
      },
    ));

    // Find the login button
    final Finder MenuButtonFinder = find.text('Menu');
    ...
    // Verify if navigation to login page occurred
    expect(find.byType(MenuPage), findsOneWidget); //replace login with unique word on the page
}

void testVoucherButton(WidgetTester tester) async
{
    await tester.pumpWidget(MaterialApp(
      home: Scaffold(
        body: Navigationbar(total: 0.0), // Render Navigationbar on the screen for testing
      ...
      routes: {
        '/voucher': (context) => VoucherPage(), // replace LoginPage with the class that defines the page
      },
    ));

    // Find the login button
    final Finder VoucherButtonFinder = find.text('Voucher');

    // Tap the login button
    await tester.tap(VoucherButtonFinder);
    await tester.pumpAndSettle(); // Wait for navigation to complete

    // Verify if navigation to login page occurred
    expect(find.byType(VoucherPage), findsNWidgets(3)); //replace login with unique word on the page
}